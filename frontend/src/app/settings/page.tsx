      ...
      <main className="flex-1 pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        ...
        <div className="space-y-8">
          ...
          <motion.section 
            ...
            className="glass-card rounded-[2rem] p-8 md:p-10 border-white/5"
          >
            <h2 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-10 flex items-center gap-3">
              <User className="w-5 h-5 text-indigo-400" />
              Account Identity
            </h2>
            <div className="flex flex-col md:flex-row items-center gap-10">
              <div className="relative group">
                ...
                {user?.photoURL ? (
                  <img src={user.photoURL} alt="Profile" className="relative w-28 h-28 rounded-3xl border border-white/10 shadow-2xl" />
                ) : (
                  <div className="relative w-28 h-28 bg-slate-900 rounded-3xl flex items-center justify-center border border-white/10 shadow-2xl">
                    <User className="w-12 h-12 text-slate-700" />
                  </div>
                )}
              </div>
              <div className="text-center md:text-left flex-1">
                <h3 className="text-3xl font-bold text-white mb-2">{user?.displayName}</h3>
                <p className="text-slate-400 text-lg mb-4">{user?.email}</p>
                <div className="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 rounded-2xl bg-white/[0.02] border border-white/5">
              <div>
                <h4 className="font-bold text-lg text-white">Visual Theme</h4>
                <p className="text-slate-500">Currently optimized for deep-dark professional aesthetic.</p>
              </div>
              <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-white/5 shadow-inner">
                <button className="p-3 text-slate-500 hover:text-white transition-colors"><Sun className="w-5 h-5" /></button>
                <button className="p-3 bg-indigo-500 text-white rounded-xl shadow-xl shadow-indigo-500/30">
...
function SettingItem({ title, description, onClick, badge, toggle, checked }: any) {
  return (
    <div 
      onClick={onClick}
      className={`flex items-center justify-between p-6 rounded-2xl bg-white/[0.02] border border-white/5 transition-all duration-500 group ${onClick ? 'cursor-pointer hover:bg-white/[0.05] hover:border-indigo-500/30' : ''}`}
    >
      <div className="flex-1 pr-8">
        <div className="flex items-center gap-3 mb-1">
          <h4 className="font-bold text-lg text-white group-hover:text-indigo-300 transition-colors">{title}</h4>
          {badge && (
            <span className="px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest bg-slate-950 text-slate-500 border border-white/5">
              {badge}
            </span>
          )}
        </div>
      ...
      {toggle ? (
        <div className={`w-14 h-8 rounded-full p-1.5 transition-all duration-500 cursor-pointer ${checked ? 'bg-indigo-500' : 'bg-slate-800'}`}>
        ...
        <div className="w-10 h-10 rounded-xl bg-slate-950 border border-white/5 flex items-center justify-center group-hover:border-indigo-500/30 transition-all">
